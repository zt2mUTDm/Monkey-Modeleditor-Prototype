...
public final class ThreeDimensionalView {
	...
	public void setObject(final SpatialEditable object) {
		addObject(object, content);
	}
	private void addObject(final SpatialEditable object, final Node parent) {
		final Spatial spatial = object.getSpatial();
		parent.attachChild(spatial);
		
		if(object instanceof NodeEditable) {
			final NodeEditable asNode = (NodeEditable) object;
			for(int i = 0, size = asNode.getChildCount(); i < size; i++) {
				addObject(asNode.getChild(i), (Node) spatial);
			}
		}
	}
	public void addObject(final SpatialEditable object, final NodeEditable parent) {
