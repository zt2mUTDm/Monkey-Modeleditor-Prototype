...
public final class PhysicsScaleEditionMode implements EditionMode {
	private final ValueIo<Vector3f> io;
	
	public PhysicsScaleEditionMode(final VehicleControlEditable parent) {
		Requires.notNull(parent, "parent == null");
		io = new ValueIo<Vector3f>() {
			@Override
			public Vector3f get() {
				return(parent.getPhysicsScale());
			}
			@Override
			public void set(final Vector3f newValue) {
				parent.setPhysicsScale(newValue);
			}
		};
	}
	public PhysicsScaleEditionMode(final RigidBodyControlEditable parent) {
		Requires.notNull(parent, "parent == null");
		io = new ValueIo<Vector3f>() {
			@Override
			public Vector3f get() {
