...
public final class AngularFactorEditionMode implements EditionMode {
	private final ValueIo<Vector3f> io;
	
	public AngularFactorEditionMode(final VehicleControlEditable parent) {
		Requires.notNull(parent, "parent == null");
		io = new ValueIo<Vector3f>() {
			@Override
			public Vector3f get() {
				return(parent.getAngularFactor());
			}
			@Override
			public void set(final Vector3f newValue) {
				parent.setAngularFactor(newValue);
			}
		};
	}
	public AngularFactorEditionMode(final RigidBodyControlEditable parent) {
		Requires.notNull(parent, "parent == null");
		io = new ValueIo<Vector3f>() {
			@Override
			public Vector3f get() {
