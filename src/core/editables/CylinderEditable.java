...
public final class CylinderEditable implements Editable, HasColor, HasAxisSamples, HasRadialSamples, HasRadius,
		HasHeight, HasClosed, HasInverted {
	private static final Collection<EditionMode> MODS = new ArrayList<>(1) {
		{
			...
			add(EditionMode.AXIS_SAMPLES);
			add(EditionMode.RADIAL_SAMPLES);
			add(EditionMode.RADIUS);
			add(EditionMode.HEIGHT);
			add(EditionMode.CLOSED);
			add(EditionMode.INVERTED);
	...
	public CylinderEditable(final SimpleApplication app, final String name) {
	...
	public CylinderEditable(final SimpleApplication app, final Geometry geometry) {
		this.app = Requires.notNull(app, "app == null");
		
		final Cylinder mesh = (Cylinder) geometry.getMesh();
		
		axisSamples = mesh.getAxisSamples();
		radialSamples = mesh.getRadialSamples();
		radius = mesh.getRadius();
		height = mesh.getHeight();
		closed = mesh.isClosed();
		inverted = mesh.isInverted();
	...
	private Mesh createMesh() {
		return(new Cylinder(axisSamples, radialSamples, radius, height, closed, inverted));
	}
	...
	public void setAxisSamples(final int axisSamples) {
		if(axisSamples != this.axisSamples) {
