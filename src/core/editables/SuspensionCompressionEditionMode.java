...
public final class SuspensionCompressionEditionMode implements EditionMode {
	private final ValueIo<Float> io;
	
	public SuspensionCompressionEditionMode(final VehicleControlEditable parent) {
		Requires.notNull(parent, "parent == null");
		io = new ValueIo<Float>() {
			@Override
			public Float get() {
				return(parent.getSuspensionCompression());
			}
			@Override
			public void set(final Float newValue) {
				parent.setSuspensionCompression(newValue);
			}
		};
	}
