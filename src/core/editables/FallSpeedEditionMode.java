...
public final class FallSpeedEditionMode implements EditionMode {
	private final ValueIo<Float> io;
	
	public FallSpeedEditionMode(final CharacterControlEditable parent) {
		Requires.notNull(parent, "parent == null");
		io = new ValueIo<Float>() {
			@Override
			public Float get() {
				return(parent.getFallSpeed());
			}
			@Override
			public void set(final Float newValue) {
				parent.setFallSpeed(newValue);
			}
		};
	}
