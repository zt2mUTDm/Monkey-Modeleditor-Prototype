...
public final class MaxSuspensionForceEditionMode implements EditionMode {
	private final ValueIo<Float> io;
	
	public MaxSuspensionForceEditionMode(final VehicleControlEditable parent) {
		Requires.notNull(parent, "parent == null");
		io = new ValueIo<Float>() {
			@Override
			public Float get() {
				return(parent.getMaxSuspensionForce());
			}
			@Override
			public void set(final Float newValue) {
				parent.setMaxSuspensionForce(newValue);
			}
		};
	}
