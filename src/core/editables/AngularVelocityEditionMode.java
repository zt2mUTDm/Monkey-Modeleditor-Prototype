...
public final class AngularVelocityEditionMode implements EditionMode {
	private final ValueIo<Vector3f> io;
	
	public AngularVelocityEditionMode(final VehicleControlEditable parent) {
		Requires.notNull(parent, "parent == null");
		io = new ValueIo<Vector3f>() {
			@Override
			public Vector3f get() {
				return(parent.getAngularVelocity());
			}
			@Override
			public void set(final Vector3f newValue) {
				parent.setAngularVelocity(newValue);
			}
		};
	}
	public AngularVelocityEditionMode(final RigidBodyControlEditable parent) {
		Requires.notNull(parent, "parent == null");
		io = new ValueIo<Vector3f>() {
			@Override
			public Vector3f get() {
	...
	public AngularVelocityEditionMode(final CharacterControlEditable parent) {
		Requires.notNull(parent, "parent == null");
		io = new ValueIo<Vector3f>() {
			@Override
			public Vector3f get() {
