...
public final class InverseInertiaLocalEditionMode implements EditionMode {
	private final ValueIo<Vector3f> io;
	
	public InverseInertiaLocalEditionMode(final VehicleControlEditable parent) {
		Requires.notNull(parent, "parent == null");
		io = new ValueIo<Vector3f>() {
			@Override
			public Vector3f get() {
				return(parent.getInverseInertiaLocal());
			}
			@Override
			public void set(final Vector3f newValue) {
				parent.setInverseInertiaLocal(newValue);
			}
		};
	}
	public InverseInertiaLocalEditionMode(final RigidBodyControlEditable parent) {
		Requires.notNull(parent, "parent == null");
		io = new ValueIo<Vector3f>() {
			@Override
			public Vector3f get() {
