...
public final class JumpSpeedEditionMode implements EditionMode {
	private final ValueIo<Float> io;
	
	public JumpSpeedEditionMode(final CharacterControlEditable parent) {
		Requires.notNull(parent, "parent == null");
		io = new ValueIo<Float>() {
			@Override
			public Float get() {
				return(parent.getJumpSpeed());
			}
			@Override
			public void set(final Float newValue) {
				parent.setJumpSpeed(newValue);
			}
		};
	}
