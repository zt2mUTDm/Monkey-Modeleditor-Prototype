...
public class ApplyScaleEditionMode implements EditionMode {
	private final ValueIo<Boolean> io;
	
	public ApplyScaleEditionMode(final GhostControlEditable parent) {
		Requires.notNull(parent, "parent == null");
		io = new ValueIo<Boolean>() {
			@Override
			public Boolean get() {
				return(parent.isApplyScale());
			}
			@Override
			public void set(final Boolean newValue) {
				parent.setApplyScale(newValue);
			}
		};
	}
