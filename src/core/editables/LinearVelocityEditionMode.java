...
public final class LinearVelocityEditionMode implements EditionMode {
	private final ValueIo<Vector3f> io;
	
	public LinearVelocityEditionMode(final VehicleControlEditable parent) {
		Requires.notNull(parent, "parent == null");
		io = new ValueIo<Vector3f>() {
			@Override
			public Vector3f get() {
				return(parent.getLinearVelocity());
			}
			@Override
			public void set(final Vector3f newValue) {
				parent.setLinearVelocity(newValue);
			}
		};
	}
	public LinearVelocityEditionMode(final RigidBodyControlEditable parent) {
		Requires.notNull(parent, "parent == null");
		io = new ValueIo<Vector3f>() {
			@Override
			public Vector3f get() {
	...
	public LinearVelocityEditionMode(final CharacterControlEditable parent) {
		Requires.notNull(parent, "parent == null");
		io = new ValueIo<Vector3f>() {
			@Override
			public Vector3f get() {
